...
@Slf4j
@Component
public class B2BCenterComplainProcessMQSender implements RabbitTemplate.ConfirmCallback {
    ...
    @Autowired
    public B2BCenterComplainProcessMQSender(RabbitTemplate kklRabbitTemplate, RetryTemplate kklRabbitRetryTemplate) {
     ...
     * @param message 消息体
     */
    public void send(MQB2BOrderComplainProcessMessage.B2BOrderComplainProcessMessage message) {
        try {
            retryTemplate.execute((RetryCallback<Object, Exception>) context -> {
                ...
                rabbitTemplate.convertAndSend(
                        B2BMQConstant.MQ_B2BCENTER_COMPLAIN_PROCESS_CALLBACK_DELAY,
                        B2BMQConstant.MQ_B2BCENTER_COMPLAIN_PROCESS_CALLBACK_DELAY,
                ...
                log.error("MQ_B2BCENTER_COMPLAIN_PROCESS_CALLBACK_DELAY {}, {}", throwable.getLocalizedMessage(), msg);
                String msgJson = new JsonFormat().printToString(msg);
                sysLogService.insert(1L,msgJson,"调用投诉单处理队列失败：" + throwable.getLocalizedMessage(),
                        "调用投诉单处理队列失败", B2BMQConstant.MQ_B2BCENTER_COMPLAIN_PROCESS_CALLBACK_DELAY, "");
                return null;
            });
        } catch (Exception e) {
            String msgJson = new JsonFormat().printToString(message);
            sysLogService.insert(1L,msgJson,"调用投诉单处理队列失败：" + e.getMessage(),
                    "调用投诉单处理队列失败", B2BMQConstant.MQ_B2BCENTER_COMPLAIN_PROCESS_CALLBACK_DELAY, "");
        }
