...
@Slf4j
@Component
public class B2BCenterReminderProcessMQSender implements RabbitTemplate.ConfirmCallback {
    ...
    @Autowired
    public B2BCenterReminderProcessMQSender(RabbitTemplate kklRabbitTemplate, RetryTemplate kklRabbitRetryTemplate) {
     ...
     * @param message 消息体
     */
    public void send(MQB2BOrderReminderProcessMessage.B2BOrderReminderProcessMessage message) {
        try {
            retryTemplate.execute((RetryCallback<Object, Exception>) context -> {
                ...
                log.error("MQ_B2BCENTER_PROCESS_B2BORDER_REMINDER_RETRY {}, {}", throwable.getLocalizedMessage(), msg);
                String msgJson = new JsonFormat().printToString(msg);
                sysLogService.insert(1L,msgJson,"调用催单处理队列失败：" + throwable.getLocalizedMessage(),
                        "调用催单处理队列失败", B2BMQConstant.MQ_B2BCENTER_PROCESS_B2BORDER_REMINDER_RETRY, "");
                return null;
            });
        } catch (Exception e) {
            String msgJson = new JsonFormat().printToString(message);
            sysLogService.insert(1L,msgJson,"调用催单处理队列失败：" + e.getMessage(),
                    "调用催单处理队列失败", B2BMQConstant.MQ_B2BCENTER_PROCESS_B2BORDER_REMINDER_RETRY, "");
        }
